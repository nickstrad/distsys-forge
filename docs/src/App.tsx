...
import { site, siteMark, siteFolio, readerPreferencesKey } from "./site";
...
function RoadmapStatus({ roadmap }: { roadmap: Roadmap }) {
  ...
  return (
    <div className={`roadmap-status roadmap-status--${roadmap.status}`}>
      ...
      <div>
        <strong>{completed ? "Completed" : "Future state"}</strong>
        <p>
          {completed
            ? `${completedOn ? `Marked complete ${completedOn}.` : "Marked complete."} This lesson is retained as history; current code and tests are authoritative where delivery differs.`
            : "This lesson is planned or in progress. The codebase is the authority for what exists today."}
        </p>
      </div>
    </div>
  );
      ...
      >
        <header className="brand">
          <button
            ...
            title={collapsed ? `${site.name} home` : undefined}
          >
            <span>{siteMark}</span>
          </button>
          <div>
        ...
        <nav id="document-navigation" className="document-nav" aria-label="Lessons">
          {filtered.length ? (
              ...
              {filtered.map((document) => (
                ...
                >
                  ...
                  {document.roadmap.status === "completed" ? (
                    <span className="nav-item__complete" title="Completed lesson">
                      <CheckIcon />
...
function App() {
  ...
  useEffect(() => {
    document.title = `${active.title} · ${site.name}`;
  }, [active]);
  ...
  const categoryLabel =
    active.roadmap.status === "completed" ? "Completed lesson" : "Lesson";
      ...
      <main className="reader">
        ...
        <aside className="outline">
          ...
          <p className="outline__folio">
            {siteFolio} / {String(activeIndex + 1).padStart(2, "0")}
          </p>
        </aside>
      </main>
