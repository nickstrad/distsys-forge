...
description: This skill should be used when the user asks to "create a learning path", "start a new learning-path project", "make a new path from the template", or wants to design a tiered Go learning project (toy primitives → production primitives → capstone) from ~/Software/distsys-forge. Bootstraps the project with a deterministic script, interviews the user, then generates the lesson sequence.
---

# Create a learning path

Instantiate `~/Software/distsys-forge` as a new project, interview the user to
...
```sh
"${LEARNING_PATH_TEMPLATE:-$HOME/Software/distsys-forge}/scripts/new-learning-path.sh" <slug> "<Display Name>"
```
